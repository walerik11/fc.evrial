...
{
    uint32 quality = ItemPrototype().Quality;
    if (quality == ITEM_QUALITY_UNCOMMON || quality == ITEM_QUALITY_RARE || quality == ITEM_QUALITY_EPIC || quality == ITEM_QUALITY_LEGENDARY || quality == ITEM_QUALITY_ARTIFACT)
        return true;
    return false;
}